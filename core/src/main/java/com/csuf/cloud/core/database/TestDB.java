...
public class TestDB implements WorkflowProcess {
			...
			try {
						...
						if (nNode.getNodeType() == org.w3c.dom.Node.ELEMENT_NODE) {
							try {
								...
								for (Route backRoute : backRoutesList) {
									backRoutesJson.put(backRoute.getName());
								}
								String saveHistoryBackRoute = "";
								if (backRoutesJson.length() == 1) {
				...
				/*if (conn != null) {
				  ...
				  if (dataMap != null) {
	                    ...
	                    //On-Prem
	                    String dbServiceUrl = "https://myformstst.fullerton.edu/bin/db/save";
